...
| [#6](https://github.com/AdityaGaur77/Agent-harness-26/pull/6) | Add synthetic demo database baseline | Merged (`7316e9c`) | Reviewed after `/agentic_review`. The schema difference is intentional and documented: the canonical demo keeps the non-null customer parent and anonymises its PII so the protected orders and order items survive. The primary `mcp-server` remains on `blast_main`; the separately named `demo-mcp-server` targets `blast_demo`. Both strategies are isolated and covered by their own runbooks. Qodo's persistent review updated through the final feature commit before the branch-sync merge. |
| [#14](https://github.com/AdityaGaur77/Agent-harness-26/pull/14) | Skill, acceptance script, README, submission docs (consolidated from closed #8-13) | Merged (571a222) | 1 High + 2 Medium found, all fixed in commit `1771ae1`, confirmed `✓ Resolved` by Qodo's follow-up pass. No dismissals. |
| [#18](https://github.com/AdityaGaur77/Agent-harness-26/pull/18) | Connect UI to the live TrueForge harness | Open (mergeable) | Latest pass found 1 Medium documentation finding because this sweep omitted PR #18; fixed in the follow-up commit and awaiting Qodo confirmation. The prior implementation pass reported 0 bugs, 0 rule violations, and 0 skill insights. |
