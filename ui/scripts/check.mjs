...
const assertions = [
  ...
  ["prompt-first home", html.indexOf('id="home-prompt"') < html.indexOf('id="recent-missions"') && html.includes("What would you like me to find or remove?")],
  ["only relevant chat patterns", irrelevantChatFeatures.every((value) => !all.includes(value))],
  ["home navigation is privacy specific", ["New request", "Activity", "Monitored information", "Connections"].every((value) => html.includes(value))],
  ["Merriweather is the agent voice", html.includes("family=Merriweather") && css.includes("--font-agent") && css.includes("Merriweather")],
  ["readable neutral token system", ["--canvas", "--surface", "--ink", "--muted", "--line", "--accent", "--accent-soft"].every((token) => css.includes(token)) && css.includes("oklch")],
  ...
  ["humanized product copy", ["Tell me what worries you", "We think this is you", "What would change", "Safety check", "Runs on its own"].every((value) => html.includes(value))],
  ...
  ["autonomy continues after confirmation", html.includes("Runs on its own") && js.includes("continueAutonomousRun") && js.includes("standingAuthorization")],
  ...
  ["what would change and exact rehearsal", html.includes('id="impact-summary"') && all.includes("42") && all.includes("33") && all.includes("24") && all.includes("9") && /0 conflicts/i.test(all)],
  ["all seven harness tools represented", toolNames.every((name) => all.includes(name))],
  ["technical details are optional", html.includes('id="technical-details"') && html.includes("Technical details") && /compacted/i.test(all)],
  ...
  ["scope and rollback preserved", html.includes('id="scope-policy"') && /scope clamp/i.test(all) && /rollback/i.test(all)],
  ...
  ["demo is honest and human", html.includes("This demo uses made-up information") && html.includes("Nothing changes real data")],
  ...
  ["responsive navigation and drawer", css.includes("@media (max-width: 1040px)") && css.includes("@media (max-width: 720px)") && html.includes('id="sidebar-toggle"') && html.includes('id="details-toggle"')],
