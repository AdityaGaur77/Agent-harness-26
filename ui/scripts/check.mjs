...
const assertions = [
  ...
  ["live connection controls are usable", html.includes('id="connector-dialog"') && html.includes('id="connector-url"') && html.includes('id="connector-token"') && html.includes('id="connector-test"') && html.includes('id="connector-result"') && js.includes("/healthz") && js.includes('"tools/list"') && js.includes('"execute_deletion"')],
  ...
  ["indeterminate presence stays finite", js.includes('"indeterminate"') && js.includes("indeterminate: 0.12")],
  ...
  ["approval uses its reviewed connector", js.includes("connector: { url: harnessUrl, token: harnessToken }") && js.includes("approval_connector_changed") && js.includes("mcpTool(executionConnector.url, executionConnector.token")],
  ["indeterminate execution is reconciled", js.includes("executionDispatched") && js.includes("executionIndeterminate") && js.includes('"find_subject_data"') && js.includes("Outcome needs confirmation") && js.includes("cannot be retried safely")],
  ["definitive execution failures stay known", js.includes("definitiveNoChange") && js.includes("executionDispatched && !definitiveNoChange") && js.includes("The service rolled back the approved request")],
  ["stale reconciliation is suppressed", js.includes("executionGeneration") && js.includes("if (executionGeneration !== run.generation) return;")],
